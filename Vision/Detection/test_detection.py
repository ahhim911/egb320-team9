...
color_ranges = {
    ...
    'Ramp': (np.array([0, 100, 200]), np.array([50, 255, 255]))
}
...
def detect_in_video(video_path):
    ...
    # Initialize detectors
    shelf_detector = Shelf(homography_matrix=homography_matrix,draw=True)
    marker_detector = Marker()
    wall_detector = Wall(homography_matrix=homography_matrix,draw=True)
    ramp_detector = PackingStationRamp(focal_length=300, homography_matrix=homography_matrix,draw=True)  # Initialize the ramp detector
    obstacle_detector = Obstacle(focal_length=300, homography_matrix=homography_matrix,draw=True)
    item_detector = Item(draw=True)
    ...
    while cap.isOpened():
        ...
        detected_walls, wall_frame, filled_wall_mask = wall_detector.find_wall(frame, color_ranges)
...
def main():
    ...
    file_path = '/home/edmond/egb320-team9/Vision/Camera/Videos/1_searching_left_1.mp4'
