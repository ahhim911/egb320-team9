...
def capture_frames(video_capture):
    """Capture frames from the video file and store them in a global variable."""
    global frame
    while True:
        ret, local_frame = video_capture.read()
        if not ret:
            break  # End of video
        with frame_lock:
            frame = local_frame
...
def main():
    ...
    video_file_path = 'your_video_file.mp4'  # Replace with the path to your video file
